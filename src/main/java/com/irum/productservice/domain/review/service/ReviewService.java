...
public class ReviewService {
    ...
    public ReviewResponse createReview(ReviewCreateRequest request) {
        ...
        if (!"DELIVERED".equals(orderDetail.status()))
        ...
        Product product = productRepository.findById(orderDetail.productId())
                .orElseThrow(() -> new CommonException(ReviewErrorCode.PRODUCT_NOT_FOUND));
        ...
        List<ReviewImage> images = (request.imageUrls() == null)
                ? List.of()
        ...
        log.info("리뷰 생성 완료: memberId={}, orderDetailId={}, productId={}",
                currentMemberId, orderDetail.orderDetailId(), product.getId());
    ...
    public ReviewResponse updateReview(UUID reviewId, ReviewUpdateRequest request) {
        ...
        if (request.imageUrls() != null) {
            ...
            List<ReviewImage> newImages = request.imageUrls().stream()
                    .map(url -> ReviewImage.create(url, review))
                    .toList();
            reviewImageRepository.saveAll(newImages);
        }

        List<String> imageUrls = reviewImageRepository.findAllByReview(review).stream()
    ...
    public Page<ReviewResponse> getMyReviews(Pageable pageable) {
        ...
        return reviewRepository.findAllByMemberId(currentMemberId, pageable)
                .map(review -> {
                    List<String> urls = reviewImageRepository.findAllByReview(review).stream()
                            .map(ReviewImage::getImageUrl)
                            .toList();
